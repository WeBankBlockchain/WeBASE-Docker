...
parse_params()
{
    while getopts "c:a:g:h" option;do
        ...
        c) ca_path="${OPTARG}"
            ...
            [ -f "$ca_path/root.crt" ] && root_crt="${output_dir}/cert/root.crt"
        ;;
        a) agency="${OPTARG}"
        ...
        g) guomi_mode="yes" && gmca_path=$OPTARG
            ...
            [ -f "$gmca_path/gmroot.crt" ] && gmroot_crt="${output_dir}/gmcert/gmroot.crt"
        ;;
        h) help;;
...
gen_agency_cert() {
    ...
    if [[ -n "${root_crt}" ]];then
        ...
        cp "$chain/ca.crt" "$chain/cert.cnf" "$certpath/"
    fi
